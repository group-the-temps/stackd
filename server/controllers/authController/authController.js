...
const register = async (req, res) => {
    ...
    if (checkUser.length === 0 && STUDENT_INVITE_CODE === student_invite_code) {
        ...
        const user = await db.auth.register_student_user([email, hashedPassword, display_name]);
        req.session.user = {
        ...
        const salt = bcrypt.genSaltSync(10);
        const hashedPassword = bcrypt.hashSync(password, salt);
        const user = await db.auth.register_admin_user([email, hashedPassword, display_name]);
        req.session.user = {
            user_id: user[0].user_id,
            email,
            display_name,
